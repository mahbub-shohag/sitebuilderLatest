...
GROUP BY
  ...
  public function bill_preview_list($bill_id)
  {
    $result = $this->db->query('SELECT
                    ...
                    LEFT JOIN BILL_ATTACHMENT ON "bill_details".BILL_DETAILS_ID = BILL_ATTACHMENT.BILL_DETAILS_ID
                    LEFT JOIN "product" ON "bill_details".PRODUCT_ID = "product".PRODUCT_ID
                    left JOIN "bill" ON "bill_details".BILL_ID = "bill".BILL_ID
                    left JOIN "supplier" ON "bill".SUPPLIER_ID = "supplier".SUPPLIER_ID
                WHERE
                "bill_details".BILL_ID = \'' . $bill_id . '\' AND
                "bill_details".STATUS != 13');
    return $result->result_array();
  }
...
');
  ...
  public function get_attachments($request_id = '', $attachment_for = '', $module_id = '')
  {
    $this->db->select('*');
    ...
    if (!empty($attachment_for)):
      $this->db->where('ATTACHMENT_FOR', $attachment_for);
    endif;
    if (!empty($module_id)):
