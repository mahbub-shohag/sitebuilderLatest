...
class Payment_Model extends CI_Model
{
 ...
 public function get_bill_details($BILL_ID=null)
  ...
  public function get_details_item($bill_id, $item_details_id)
  {
    $this->db->select('*');
    $this->db->from('bill_details');
    $this->db->where('BILL_ID', $bill_id);
    $this->db->where('ITEM_DETAILS_ID', $item_details_id);
    $result = $this->db->get();
    return $result->result_array();
  }
...
GROUP BY
  ...
  public function get_bill_item_list($bill_id)
  {
	...
	"item_details".EXCHANGE_RATE,
	"item_details".PURCHASE_DATE,
	"item_details".MEMO_NUMBER,
	"item_details".PO_NUMBER,
	"item_details".VAT_METHOD,
	"item_details".TAX_METHOD,
...
FROM
	"item_details"
LEFT JOIN "product" ON "item_details".PRODUCT_ID = "product".PRODUCT_ID
LEFT JOIN "BILL_ATTACHMENT" ON "item_details".ITEM_DETAILS_ID = BILL_ATTACHMENT.ITEM_DETAILS_ID
WHERE
	"item_details".BILL_ID = \'' . $bill_id . '\'
AND "item_details".STATUS != 13
GROUP BY
	"item_details".BILL_ID,
	"item_details".ITEM_ID,
	"item_details".DETAILS,
  ...
  public function bill_preview_list($bill_id)
  {
    $result = $this->db->query('SELECT
                    ...
                    LEFT JOIN BILL_ATTACHMENT ON "item_details".ITEM_DETAILS_ID = BILL_ATTACHMENT.ITEM_DETAILS_ID
                    LEFT JOIN "product" ON "item_details".PRODUCT_ID = "product".PRODUCT_ID
                    left JOIN "bill" ON "item_details".BILL_ID = "bill".BILL_ID
                    left JOIN "supplier" ON "bill".SUPPLIER_ID = "supplier".SUPPLIER_ID
                WHERE
                "item_details".BILL_ID = \'' . $bill_id . '\' AND
                "item_details".STATUS != 13');
    return $result->result_array();
  }

  public function get_details_item_info($bill_id, $item_details_id=null)
  {
    ...
    if($item_details_id){
        $this->db->where('bill_details.ITEM_DETAILS_ID', $item_details_id);
    }
    $result = $this->db->get();
     if($item_details_id){
        return $result->row();
