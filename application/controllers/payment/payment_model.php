...
class Payment_Model extends CI_Model
{
  ...
  public function get_bill($bill_id = NULL)
  {
      ...
      $result = $this->db->select('
                        ...
                        "bill".PAN_ID,
                        "bill".BILL_FOR_ID,
                        "bill".VAT_TAX_METHOD_ID,
                        "bill".PO_NUMBERS,
                        "bill".PG_APG,
                        "bill".INVOICE_DATE,
                        "bill".PENALTY_AMOUNT,
                        "bill".ADVANCE_ADJUSTMENT,
                        "bill".SECURITY_DEPOSIT_AMOUNT,
                        "bill".COMMISSION_AMOUNT,
                        "bill".NET_PAYMENT_AMOUNT,
                        "bill".PURPOSE,
                        "bill".REMARKS,
                        "bill".PAYMENT_TYPE_FOR_OTHER,
                        "bill".COMMISSION_PERCENTAGE_AMOUNT,
                       ...
                       ) TOTAL_BILL,VAT_TAX_METHOD.METHOD_NAME',false);
      $this->db->from('bill');
      $this->db->join('payment_approval_note', 'bill.PAN_ID = payment_approval_note.PAN_ID', 'left');
      ...
      $this->db->join('bill_details', 'bill_details.BILL_ID = bill.BILL_ID', 'left');
      $this->db->join('VAT_TAX_METHOD', 'bill.VAT_TAX_METHOD_ID = VAT_TAX_METHOD.VAT_TAX_METHOD_ID', 'left');
      $this->db->join('status', 'bill.STATUS = status.STATUS_ID', 'left');
      if($bill_id){
         $this->db->where('bill.BILL_ID', $bill_id); 
      }
      $this->db->group_by(' bill .BILL_ID,
                            bill .SUPPLIER_ID,
                            bill .INVOICE_NUMBER,
                            bill .REFERENCE_NUMBER,
                            bill .PURCHASE_DATE,
                            ...
                            supplier . SUPPLIER_NAME ,
                            payment_type . PAY_TYPE_NAME ,
                            status.STATUS_NAME,VAT_TAX_METHOD.METHOD_NAME'); 
      $result = $this->db->get();
  ...
  {
    $this->db->select('bill.*, supplier.SUPPLIER_NAME, payment_type.PAY_TYPE_NAME, status.STATUS_NAME, payment_approval_note.BILL_ID "PAN_BILL_ID"');
    $this->db->from('bill');
    $this->db->join('supplier', 'bill.SUPPLIER_ID = supplier.SUPPLIER_ID', 'LEFT');
    $this->db->join('payment_type', 'bill.PAYMENT_TYPE = payment_type.PAY_TYPE_ID', 'LEFT');
    $this->db->join('status', 'bill.STATUS = status.STATUS_ID', 'LEFT');
    $this->db->join('payment_approval_note', 'bill.PAN_ID = payment_approval_note.PAN_ID', 'LEFT');
  ...
  public function get_bills_for_pan($where_in)
  {
    $this->db->select('bill.*');
    $this->db->from('bill');
    $this->db->where_in('bill.BILL_ID', $where_in);
    return $this->db->get();
  }
  
  public function get_single_bill($bill_id)
  {
    $this->db->select('B.*, S.SUPPLIER_NAME, PT.PAY_TYPE_NAME, BF.BILL_FOR_NAME, VTM.METHOD_NAME');
    $this->db->from('bill "B"');
    $this->db->where('B.BILL_ID', $bill_id);
    $this->db->join('supplier "S"', 'B.SUPPLIER_ID = S.SUPPLIER_ID', 'LEFT');
    $this->db->join('payment_type "PT"', 'B.PAYMENT_TYPE = PT.PAY_TYPE_ID', 'LEFT');
    $this->db->join('BILL_FOR "BF"', 'B.BILL_FOR_ID = BF.BILL_FOR_ID', 'LEFT');
    $this->db->join('VAT_TAX_METHOD "VTM"', 'B.VAT_TAX_METHOD_ID = VTM.VAT_TAX_METHOD_ID', 'LEFT');
    $this->db->where('ROWNUM = ', 1, FALSE);
    return $this->db->get();
  }
  
   /* Start Of Mahbub Block*/

 public function get_bill_details($BILL_ID=null)
  {
    $this->db->select('bill_details.* , product.PRODUCT_NAME');
    $this->db->from('bill_details');
    $this->db->join('product', 'bill_details.PRODUCT_ID = product.PRODUCT_ID' ,'left');
    if($BILL_ID )
    {
  ...
  public function get_details_item($bill_id, $bill_details_id)
  {
    $this->db->select('*');
    $this->db->from('bill_details');
    $this->db->where('BILL_ID', $bill_id);
    $this->db->where('BILL_DETAILS_ID', $bill_details_id);
    $result = $this->db->get();
    return $result->result_array();
  }
  ...
  public function get_pan_info_prev($bill_id = '')
  {
	...
	SUM (
		NVL (
			...
			0
		) * NVL ("bill_details"."PRICE", 0) + NVL ("bill_details"."VAT", 0) + NVL ("bill_details"."TAX", 0)
	) TOTAL_BILL
FROM
	"bill"
JOIN "supplier" ON "bill"."SUPPLIER_ID" = "supplier"."SUPPLIER_ID"
JOIN "payment_type" ON "payment_type"."PAY_TYPE_ID" = "bill"."PAYMENT_TYPE"
JOIN "bill_details" ON "bill_details"."BILL_ID" = "bill"."BILL_ID"
...
GROUP BY
  ...
  public function get_pan_info($bill_id = '')
  {
    ...
    $sql = 'SELECT
	SUM (
		NVL ("bill"."INVOICE_AMOUNT", 0)
	) "TOTAL_BILL",
    (SELECT SUM("bill_details".TOTAL_PRICE) FROM "bill_details" WHERE "bill_details".BILL_ID IN (' . $final_bill_id . ')) "PAID_UP_TO_DATE",
	SUM (
...
GROUP BY
  ...
  public function get_payment_approval_note($pan_id = '')
  {
    $this->db->select('payment_approval_note.*, currency.CURRENCY_NAME,bill.SUPPLIER_ID,supplier.SUPPLIER_NAME  "SUP_NAME"');
    $this->db->from('payment_approval_note');
    $this->db->join('currency', 'payment_approval_note.CURRENCY_ID = currency.CURRENCY_ID', 'LEFT');
    $this->db->join('bill', 'payment_approval_note.BILL_ID = bill.BILL_ID', 'LEFT');
    $this->db->join('supplier', 'bill.SUPPLIER_ID = supplier.SUPPLIER_ID', 'LEFT');
    if (!empty($pan_id)):
      $this->db->where('payment_approval_note.PAN_ID', $pan_id);
      $this->db->where('ROWNUM = ', 1, FALSE);
    endif;
    return $this->db->get();
  }
  ...
  {
    $this->db->select('bill_details.*,items.ITEM_NAME');
    $this->db->from('bill_details');
    $this->db->join('items', 'bill_details.ITEM_ID=items.ITEM_ID');
  ...
  public function get_bill_item_list($bill_id)
  {
	...
	"bill_details".VAT,
	"bill_details".TAX,
	"bill_details".CURRENCY,
	"bill_details".EXCHANGE_RATE,
	"bill_details".PURCHASE_DATE,
	"bill_details".MEMO_NUMBER,
	"bill_details".PO_NUMBER,
	"bill_details".VAT_METHOD,
	"bill_details".TAX_METHOD,
	"product".PRODUCT_NAME,
	COUNT (
		BILL_ATTACHMENT.FILE_NAME
	) NOF
FROM
	"bill_details"
LEFT JOIN "product" ON "bill_details".PRODUCT_ID = "product".PRODUCT_ID
LEFT JOIN "BILL_ATTACHMENT" ON "bill_details".BILL_DETAILS_ID = BILL_ATTACHMENT.BILL_DETAILS_ID
WHERE
	"bill_details".BILL_ID = \'' . $bill_id . '\'
AND "bill_details".STATUS != 13
GROUP BY
	"bill_details".BILL_ID,
	"bill_details".ITEM_ID,
	"bill_details".DETAILS,
  ...
  {
    $this->db->select('BD.*, P.PRODUCT_NAME, S.SUPPLIER_NAME, S.SUPPLIER_ADDRESS, S.PHONE');
    $this->db->from('bill_details "BD"');
    $this->db->join('product "P"', 'BD.PRODUCT_ID = P.PRODUCT_ID', 'LEFT');
    $this->db->join('bill "B"', 'BD.BILL_ID = B.BILL_ID', 'LEFT');
    $this->db->join('supplier "S"', 'B.SUPPLIER_ID = S.SUPPLIER_ID', 'LEFT');
    $this->db->where('BD.BILL_ID', $bill_id);
    $this->db->where('BD.STATUS <> ', 13);
    return $this->db->get();
  }

  public function get_details_item_info($bill_id, $bill_details_id=null)
  {
    $this->db->select('bill_details.*,
            product.PRODUCT_NAME,
            MEMO_TYPE.MEMO_TYPE_NAME,
            VAT_TAX_METHOD.METHOD_NAME
            
            ');
    $this->db->from('bill_details');
    $this->db->join('product','bill_details.PRODUCT_ID=product.PRODUCT_ID','left');
    $this->db->join('MEMO_TYPE','MEMO_TYPE.MEMO_TYPE_ID=bill_details.MEMO_TYPE','left');
    $this->db->join('VAT_TAX_METHOD', 'bill_details.VAT_TAX_METHOD_ID = VAT_TAX_METHOD.VAT_TAX_METHOD_ID', 'left');
    $this->db->where('bill_details.BILL_ID', $bill_id);
    if($bill_details_id){
        $this->db->where('bill_details.BILL_DETAILS_ID', $bill_details_id);
    }
    $result = $this->db->get();
     if($bill_details_id){
        return $result->row();
...
//        exit();
  ...
  {
    $this->db->select('payment_approval_note.*, bill.INVOICE_NUMBER, bill.INVOICE_AMOUNT, MAKE_PAYMENT_TYPE.MPT_NAME');
    $this->db->from('payment_approval_note');
...
//      exit();
  ...
  public function get_bill_details_by_id($item_id)
  {
    $this->db->select('*');
    $this->db->from('items');
    //$this->db->join('attachment_title','attachment_title.ATTACHMENT_TITLE_ID=attached_document.ATTACHMENT_TITLE_ID');
  ...
  public function get_details_attach_info($bill_details_id)
  {
//        $result =  $this->db->query('SELECT COUNT(BILL_DETAILS_ID) as TOTAL_ATTACHMENT, BILL_DETAILS_ID FROM BILL_ATTACHMENT GROUP BY BILL_DETAILS_ID');
    $result = $this->db->query("SELECT * FROM BILL_ATTACHMENT where BILL_DETAILS_ID= '$bill_details_id' ");
//       echo $this->db->last_query();
//       exit();
    return $result->result_array();
  }
...
//        echo'hello' .$search_act;
    $this->db->select('"gl_main".GL_ID,"gl_main".GL_CODE,"gl_main".GL_DESCRIPTION');
    $this->db->from('"gl_main"');
...
//        exit();
  ...
  {
    $this->db->select('"gl_sub".SUB_GL_ID,
                           "gl_sub".GL_ID,
                           "gl_sub".SUB_GL_CODE,
                           "gl_main".GL_DESCRIPTION');
    $this->db->from('"gl_sub"');
    $this->db->join('"gl_main"', 'gl_sub.GL_ID = gl_main.GL_ID', 'left');
...
//        exit();
  ...
  public function find_gl_sub_gl_list_chcked($gl, $sub_gl)
  {
    ...
    $this->db->join('"gl_main"', 'gl_sub.GL_ID = gl_main.GL_ID', 'left');
    $this->db->like(array('SUB_GL_DESCRIPTION' => $sub_gl, 'GL_DESCRIPTION' => $gl));
    $this->db->group_by('"gl_sub".SUB_GL_ID,
                           "gl_sub".GL_ID,
                           "gl_sub".SUB_GL_CODE,
                           "gl_main".GL_DESCRIPTION');
    $result = $this->db->get();
...
//        echo $this->db->last_query();
  ...
  public function find_make_payment_info($pan_id)
  {
    ...
    $this->db->select('"payment_approval_note".PAN_ID,
                    "payment_approval_note".BILL_ID,
                    "payment_approval_note".INVOICE_AMOUNT,
                    "payment_approval_note".WO_VALUE,
                    "payment_approval_note".PAID_UP_TO_DATE,
                    "payment_approval_note".PAID_AMOUNT,
                    "payment_approval_note".WO_REF,
                    "payment_approval_note".PURPOSE,
                    "payment_approval_note".PAN_CODE,
                    "bill".INVOICE_NUMBER,
                    "bill".REFERENCE_NUMBER,
                    "bill".BILL_AMOUNT,
                    ...
                    "supplier".SUPPLIER_NAME');
    $this->db->from('"payment_approval_note"');
    $this->db->join('"bill"', 'payment_approval_note.PAN_ID = "bill".PAN_ID');
    $this->db->join('"supplier"', 'bill.SUPPLIER_ID = "supplier".SUPPLIER_ID', 'left');
    $this->db->where('"payment_approval_note".PAN_ID = "bill".PAN_ID');
    $this->db->where('"payment_approval_note".PAN_ID', $pan_id);
...
FROM
	"bill"
INNER JOIN "bill_details" ON "bill_details".BILL_ID = "bill".BILL_ID
...
');
  ...
  public function search_purchase_order($supplier_id, $po_number)
  {
    ...
    $this->db->like('PO.PURCHASE_ORDER_NO', $po_number);
    return $this->db->get();
  }
  ...
  public function search_purchase_order_list($where)
  {
    $this->db->select('*');
    $this->db->from('purchase_order');
    $this->db->where($where);
  ...
  public function get_vat_tax($product_id)
  {
    $this->db->select('ga.*');
    $this->db->from('gl_account "ga"');
    $this->db->join('product "p"', 'ga.GL_ACCOUNT_ID = p.GL_ID', 'LEFT');
    $this->db->where('p.PRODUCT_ID', $product_id);
    $this->db->where('ROWNUM = ', 1, FALSE);
    return $this->db->get();
  }

  public function get_attachments($where)
  {
    $this->db->select('A.*, T.ATTACHMENT_TYPE_NAME');
    $this->db->from('ATTACHMENTS "A"');
    $this->db->join('ATTACHMENT_TYPE "T"', 'A.ATTACHMENT_TYPE_ID = T.ATTACHMENT_TYPE_ID', 'LEFT');
    $this->db->where($where);
    return $this->db->get();
  }
  
  public function count_attachments($where)
  {
    $this->db->select('*');
  ...
  public function get_bill_details_for_vat_15($bill_ids)
  {
    $this->db->select('*');
    $this->db->from('bill_details');
    $this->db->where('VAT_PERCENTAGE', '15');
